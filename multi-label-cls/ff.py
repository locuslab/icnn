...
def tf_nOnes(u):
    return tf.reduce_sum(tf.cast(u, tf.int32))

yhatBinary_ = tf.round(yhat_)
F1_ = tf.reduce_mean(2*tf_nOnes(tf.mul(y_, yhatBinary_))/
                     (tf_nOnes(y_) + tf_nOnes(yhatBinary_)))
...
with tf.Session(config=config) as sess:
    ...
    for i in range(nIter):
        ...
        _, loss, trainF1 = sess.run(
            [train_step, loss_, F1_],
            feed_dict={x_: xBatch, y_: yBatch})
        trainW.writerow((i, trainF1, loss))
        trainF.flush()

        yhat, yhatBinary = sess.run([yhat_, yhatBinary_],
                                    feed_dict={x_: xBatch, y_: yBatch})
        ...
        if i % np.ceil(nTrain/args.trainBatchSz) == 0:
            ...
            testF1, testLoss = sess.run([F1_, loss_],
